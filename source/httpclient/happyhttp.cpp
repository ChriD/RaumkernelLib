...
void Connection::pump()
{
	...
	else
	{
		...
		while( used < a && !m_Outstanding.empty() )
		{
            ...
            // ChriD -->
            // When redirecting in header then clear this response and creat a new with the redirecting value
            if (r->m_Reason == "Temporary Redirect")
            {
                std::string redirectLocation = r->m_Headers.at("location");         
                delete r;
                m_Outstanding.pop_front();                
                //request("GET", redirectLocation.c_str(), headers, nullptr, 0);              
            }
            else
